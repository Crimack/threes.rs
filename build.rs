...
fn main() {
    ...
    let mut build_output_dir = out_dir;
    ...
    if !dst_target_resource_dir.exists() {
        std::fs::create_dir(dst_target_resource_dir.clone()).unwrap();
    }
...
fn copy(src_dir: &PathBuf, dst_dir: &PathBuf, file_type: &str) {
    ...
    println!("{:?}", dst_dir);
    for entry in
        std::fs::read_dir(src_dir).unwrap_or_else(|_| panic!("Can't read dir {:?}", src_dir))
    {
        ...
        if let Some(file_name) = file_name_result {
            ...
            if file_name.ends_with(file_type) {
                ...
                std::fs::copy(&entry_path, new_file_path.as_path()).unwrap_or_else(|_| {
                    panic!("Can't copy from dir {:?} to {:?}", src_dir, dst_dir)
                });
            }
        }
    }
